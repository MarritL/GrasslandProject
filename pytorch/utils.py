...
def colorEncode(labelmap, colors, mode='RGB', n_classes=3):
...
def find_constant_area(seg_label, size, patch_size):
    crop = seg_label
    count=0
    while not np.all(crop == crop[0,0]) or crop.shape != (size,size):
        #row = np.random.randint(patch_size,2*patch_size-size,1) # TODO: turn back on for only inner patch
        #col = np.random.randint(patch_size,2*patch_size-size,1) # TODO: turn back on for only inner patch
        row= np.random.randint(0,patch_size,1)
        col= np.random.randint(0,patch_size,1)
        crop = seg_label[int(row):int(row)+size,int(col):int(col)+size]
        count+=1
        if count > 14:
            row=999999
            col=999999
